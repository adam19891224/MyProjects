...
function CategoryUtils() {
    ...
    var randomColor = function () {
        var colorStr = Math.floor(Math.random() * 16777215).toString(16).toUpperCase();
    ...
    this.init = function ($element) {
        ...
        new Promise(function (resolve) {
            ...
            var proxy = {nextFrame: nextFrame, draw: draw};
            ...
            $(canvas).on({
                click: function () {
                    if (active) {
                        router.push({name: "genre", params: {typeName: active.attr("tn"), p: 1}})
                    }
                }, mousemove: function (e) {
                    var x = (e.pageX - offset.left - oWidth / 2) * scale;
                    var y = (e.pageY - offset.top - oHeight / 1.621) * scale;
                    var item, itemData;
                    active = null;
                    items.each(function () {
                        item = $(this);
                        itemData = item.data("graphic");
                        if (abs(itemData.px - x) * 2 < itemData.width && abs(itemData.py - y) * 2 < itemData.height && (active ? itemData.z < active.z : 1)) {
                            active = item
                        }
                    });
                    proxy.draw();
